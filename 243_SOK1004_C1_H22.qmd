...
```{r}
df_tibble <- as_tibble(df$dataset) %>%
...
df_tibble %>%
  ...
  xlab("År") + ylab("BNP i tusener av kroner per person") +
  xlim(2000, 2021) +
  ylim(300, 800) +
  geom_line()
```

#Memo= faste priser har gradvis steget fra 2000 - 2007/2008, tiden etter fram til 2021 ligger det på det samme området stort sett.

#BNP = Løpende priser har en bratt kurve, før det avtar litt i et års tid, til at det igjen stiger. Fra 2020 til 2021 stiger det kolossalt.

#I løpende priser vil si at man tar utgangspunkt i de faktiske prisene akkurat det året, mens i faste priser så justeres BNP etter inflajon og de faktiske forholdene i landet.
...
#Datasettet ble mer oversiktlig, pivot_wider kommandoen gjorde slik at du fikk tiden i første kolonne, BNP i andre kolonne, og MemoBNP i tredje kolonne. df_tibble er litt mindre oversiktlig hvor den tar for seg BNP først og MemoBNP etterpå.
...
```{r}
df_long <- pivot_longer(df_wide, 2:7, names_to = "var", values_to = "verdi")
```
...
```{r}
Fikk ikke til denne oppgaven, men resten er besvart
```
